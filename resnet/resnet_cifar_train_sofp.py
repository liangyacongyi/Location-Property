...
# -*- encoding: utf-8 -*-
"""
@File    :   resnet_cifar_train_sofp.py
@Contact :   liangyacongyi@163.com

@Modify Time      @Author    @Version    @Desciption
------------      -------    --------    -----------
2019/8/6 10:55AM   liangcong      1.0    Train ResNet model with CIFAR-10/100

run this file with
"python resnet_cifar_train_sofp.py --data_type=cifar_10 --_alpha1=1.
--_alpha2=0.001 --num_blocks=3 --weight_decay=0.0001"
"""
import numpy as np
...
tf.app.flags.DEFINE_float("_alpha1", 1., "constraints on L_{PE}, default:1.")
tf.app.flags.DEFINE_float("_alpha2", 0.001, "constraints on L_{SE}, default:0.001")
...
tf.app.flags.DEFINE_integer("num_blocks", 3, "layers number: 6*num_blocks+2, selected from{3,5,7,9,18}, default:3")
tf.app.flags.DEFINE_string("data_type", 'cifar_10', "cifar_10 or cifar_100, default:cifar_10")
FLAGS = tf.app.flags.FLAGS
_ALPHA1 = FLAGS._alpha1
_ALPHA2 = FLAGS._alpha2
...
print("num_layers: %d, data_type: %s, _alpha1: %f, _alpha2: %f" % (6*FLAGS.num_blocks+2, _DATA_TYPE, _ALPHA1, _ALPHA2))

# ------------------------------create relative folders-------------------------------
LOAD_PAR_PATH = "./" + _DATA_TYPE + "/" + str(6*_NUM_BLOCKS+2) + "/ce"
LOAD_MODEL_SAVE_PATH = os.path.join(LOAD_PAR_PATH, "model/")

PAR_PATH = "./" + _DATA_TYPE + "/" + str(6*_NUM_BLOCKS+2) + "/alpha1_" + str(_ALPHA1) + "/alpha2_" + str(_ALPHA2)
...
l_pe = ce_loss
loss = _ALPHA1*l_pe + _ALPHA2*l_se
# ------------------------------------------------------------------------------------
...
def train(itera, f):
    ...
    print ("num_layers: %d, data_type: %s, _alpha1: %f, _alpha2: %f"
           % (6 * FLAGS.num_blocks + 2, _DATA_TYPE, _ALPHA1, _ALPHA2))
    print >>f, "num_layers: %d, data_type: %s, _alpha1: %f, _alpha2: %f" \
               % (6 * FLAGS.num_blocks + 2, _DATA_TYPE, _ALPHA1, _ALPHA2)
